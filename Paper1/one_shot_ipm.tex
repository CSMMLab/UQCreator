...
\comment{We denote the fixed point of the classical IPM iteration \eqref{eq:momentIteration} by $\bm{\hat u}^*_{\Delta} = (\bm{\hat u}_{1}^*,\cdots,\bm{\hat u}_{N_x}^*)^T$, i.e. we have
\begin{linenomath*}\begin{align*}
...
\begin{linenomath*}\begin{align*}
\partial_{\bm{\lambda}} \bm{d}(\bm{\lambda}_j^n,\bm{\hat u}_j^n) &= \bm{I}_d - \bm{H}(\comment{\bm\lambda_j^n})^{-1} \cdot \langle \nabla\bm{u}_s(\bm{\varphi}^T\bm{\lambda}_j^n)\bm{\varphi}\bm{\varphi}^T \rangle^T - \partial_{\bm{\lambda}}\bm{H}(\comment{\bm\lambda_j^n})^{-1} \cdot \left( \langle \bm{u}_s(\bm{\varphi}^T\bm{\lambda}_j^n)\bm{\varphi}^T \rangle^T - \comment{\bm{\hat u}_j^n}\right) \\
&=- \partial_{\bm{\lambda}}\bm{H}(\comment{\bm\lambda_j^n})^{-1} \cdot \left( \langle \bm{u}_s(\bm{\varphi}^T\bm{\lambda}_j^n)\bm{\varphi}^T \rangle^T - \comment{\bm{\hat u}_j^n}\right).
\end{align*}\end{linenomath*}
\comment{Recall that if $\bm{\hat u}_j^n$ and $\bm{\lambda}_j^n$ belong to the fixed point $(\bm{\lambda}_{\comment{\Delta}}^*,\bm{\hat u}_{\comment{\Delta}}^*)$,} we have $\langle \bm{u}_s(\bm{\varphi}^T\bm{\lambda}_j^n)\bm{\varphi}^T \rangle^T = \comment{\bm{\hat u}_j^n}$, hence one obtains $\partial_{\bm{\lambda}} \bm{d}=\bm{0}$. For the block $\partial_{\bm{\hat u}} \bm{d}$, we get 
\begin{linenomath*}\begin{align*}
\partial_{\bm{\hat u}} \bm{d}(\bm{\lambda}_j^n,\bm{\hat u}_j^n) = \bm{H}(\comment{\bm\lambda_j^n})^{-1},
\end{align*}\end{linenomath*}
hence $\partial_{\bm{\hat u}} \bm{d}$ is a block diagonal matrix. Let us now look at $\partial_{\bm{\lambda}} \bm{c}$ at a fixed spatial cell $j$. \comment{To shorten notation, let us define
\begin{linenomath*}\begin{align*}
...
\begin{linenomath*}\begin{align*}
\frac{\partial \bm{c}\comment{_j}}{\partial \bm{\lambda}_{\ell}}\frac{\partial \bm{d}(\bm{\lambda}_{j-1}^{n},\bm{\hat u}_{j-1}^{n})}{\partial \bm{\hat u}} = \frac{\partial \bm{c}\comment{_j}}{\partial \bm{\lambda}_{\ell}} \bm{H}(\bm\lambda)^{-1} = \frac{\partial \bm{c}\comment{_j}}{\partial \bm{\lambda}_{\ell}} \langle \nabla\bm{u}_s(\bm{\varphi}^T\bm{\lambda}_{j-1}^n)\bm{\varphi}\bm{\varphi}^T \rangle^{-T} = \partial_{\bm{\hat u}_{\ell}}\bm{\tilde c}_j
\end{align*}\end{linenomath*}
...
With the assumptions from Theorem \ref{th:Contractive}, the One-Shot IPM converges locally. I.e. \comment{if we denote the solution at iteration $n$ as $(\bm{\lambda}_{\Delta}^{(n)},\bm{\hat u}_{\Delta}^{(n)})$}, there exists a $\delta>0$ s.t. for all starting points $(\bm{\lambda}_{\comment{\Delta}}^{(0)},\bm{\hat u}_{\comment{\Delta}}^{(0)})\in B_{\delta}(\bm{\lambda}_{\comment{\Delta}}^*,\bm{\hat u}_{\comment{\Delta}}^*)$ we have
\begin{linenomath*}\begin{align*}
\left\Vert (\bm{\lambda}\comment{_{\Delta}^{(n)}},\bm{\hat u}\comment{_{\Delta}^{(n)}}) - (\bm{\lambda}_{\comment{\Delta}}^*,\bm{\hat u}_{\comment{\Delta}}^*)\right\Vert \rightarrow 0 \qquad \text{ for } n \rightarrow \infty.
\end{align*}\end{linenomath*}
\end{theorem}
\begin{proof}
...
\end{align*}\end{linenomath*}
Since the roots of a polynomial vary continuously with its coefficients, the eigenvalues of $\bm{J}$ are continuous w.r.t $(\bm{\lambda}_{\comment{\Delta}},\bm{\hat u}_{\comment{\Delta}})$. Hence there exists an open ball with radius $\delta$ around the fixed point in which the eigenvalues remain in the interval $(-1,1)$.
\end{proof}
%\begin{remark}
