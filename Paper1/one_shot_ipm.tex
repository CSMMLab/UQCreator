...
\end{align}
which is why we have the mapping $\bm{\hat u}:\mathbb{R}^{N\times p}\to\mathbb{R}^{N\times p}$, $\bm{\hat u}(\bm{\hat \lambda}) = \bm{h}(\bm{\hat \lambda})$. Since the solution of the dual problem for a given moment vector is unique, this mapping is bijective and therefore we have an inverse function
