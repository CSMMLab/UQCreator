...
\end{align*}
The computation of the so called entropy variables $\bm{\hat\lambda}_j^n:=\bm{\hat\lambda}(\bm{\hat u}_j^n)$ requires solving the dual problem \eqref{eq:dualProblem} for the moment vector $\bm{\hat u}_{j}^{n}$. To determine the dual variables for a given moment vector $\bm{\hat{u}}$, we must minimize the cost function
\begin{align*}
...
\begin{align*}
\bm{d}(\bm{\lambda},\bm{\hat{u}}):= \bm{\lambda}-\bm{B}(\bm{\lambda})\cdot \left(\langle \bm u_{ME}(\bm{\lambda}^T\bm{\varphi})\bm{\varphi}\rangle)-\bm{\hat{u}}\right),
\end{align*}
