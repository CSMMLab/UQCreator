...
The following section presents the adaptivity strategy used in this work. Since stochastic hyperbolic problems generally experience shocks in a small portion of the space-time domain, the idea is to perform arising computations on a high accuracy level in this small area, while keeping a low level of accuracy in the remainder. The hope is to \comja{automatically select the lowest order moment capable of approximating the solution with given accuracy, i.e....} achieve the finest level accuracy with this adaptive strategy, i.e. the same error is obtained while using a significantly reduced number of unknowns \comja{in most parts of the computational domain}.
...
\If{\eqref{eq:residualSteady} fulfills the stopping criterion $\varepsilon_m^*$}
\State $m \leftarrow m+1$
\EndIf
\EndWhile
\end{algorithmic}
