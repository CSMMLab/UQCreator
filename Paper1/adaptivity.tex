...
\State $\ell_j^{n+1}\leftarrow \text{\comment{Determine Refinement Level}}\left(\bm{\hat \lambda}_j^{n+1}\right)$
\EndFor
\For{$j=1$ to $N_x$}
...
\comment{Determine Refinement Level}}\left(\bm{\lambda}_j^{n+1}\right),\ell_l^*\}$
\EndFor
\For{$j=1$ to $N_x$}
\State $\bm\ell' \leftarrow (\ell_{j-1}^n,\ell_{j}^n,\ell_{j+1}^n)^T$
