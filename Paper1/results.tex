...
\centering
	...
	\end{subfigure}
	\caption{Reference solution E$[\rho]$ and Var$[\rho]$ and the mesh close to the airfoil which is used in the computation of all presented methods. \comment{The reference solution has been computed with stochastic-Collocation using $100$ Gauss-Legendre quadrature points.}}
...
\centering
	...
	\begin{subfigure}{0.5\linewidth}
		...
		\label{fig:sub34}
	\end{subfigure}
	\caption{Comparison of the relative L$^2$-error \comment{\eqref{eq:relErrorEVar}} for the density for IPM related methods (first row) and of the best performing IPM method in comparison with SC (second row). All intrusive methods \comment{(i.e. SG, IPM, osIPM, adosIPM, readosIPM) are converged} to a residual $\varepsilon=6\cdot 10^{-6}$ \comment{(see equation \eqref{eq:residualSteady})}, whereas all non-intrusive methods \comment{(i.e. SC) are converged} to a residual $\varepsilon=1\cdot 10^{-7}$ \comment{in order to obtain an error which remains constant in time. SG and IPM are computed with Algorithm~\ref{alg:IPM} (where SG uses a quadratic entropy) adapted for steady problems. For osIPM we used Algorithm~\ref{alg:osIPM}, for adosIPM and readosIPM we use Algorithm~\ref{alg:adosIPM} with and without refinement retardation.} All computations are performed with 5 MPI threads.}
...
First, let us mention that the adaptive SG method fails, since it yields negative \comment{gas} densities during the iteration. The standard SG method \comment{without adaptivity} however preserves positivity of \comment{density}, energy and pressure \comment{in our numerical experiments. Note that in general, stochastic-Galerkin does not guarantee positivity of these quantities, since the polynomial approximation does not guarantee any solution bounds.} The change of the relative L$^2$-error during the iteration to the steady state has been recorded in Figure~\ref{fig:sub31} for the expectation value and in Figure~\ref{fig:sub32} for the variance. When comparing intrusive methods without acceleration techniques as well as SC, the following properties emerge:
\begin{itemize}
