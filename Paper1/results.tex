...
The computational domain is a circle with a diameter of $40$ meters. In the center, the NACA0012 airfoil with a length of one meter is located. The spatial mesh is composed of a coarsely discretized far field and a finely resolved region around the airfoil, since we are interested in the flow solution at the airfoil. Altogether, the mesh consists of 22361 triangular elements. \remja{Note, that the cell density of the mesh was adjusted in order to capture areas of high variance and therefore the mesh is significantly different to a mesh classically applied in computational fluid dynamics.}
