...
\begin{itemize}
\item When for example comparing the error obtained with IPM$_8^3$ and IPM$_8^4$ it can be seen that the error stagnates when the chosen quadrature is not sufficiently accurate. This behavior results from aliasing effects in the numerical flux, which dominate the accuracy level. \frama{Sollen wir hier Strang's Lemma erw\"ahnen? Also der Fehler der Quadratur sollte so gross sein wie der Fehler durch die moment truncation?}
