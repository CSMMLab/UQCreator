...
ShallowWaterClosure2D::ShallowWaterClosure2D( Settings* settings ) : Closure( settings ), _g(9.81) { _alpha = 0.1; }
...
void ShallowWaterClosure2D::U( Matrix& out, const Matrix& Lambda ) {
    for( unsigned k = 0; k < Lambda.columns(); ++k ) {
        out( 0, k ) = (0.5*(2*Lambda(0,k)*Lambda(2,k) + pow(Lambda(1,k),2)*Lambda(2,k) + pow(Lambda(2,k),3)))/(_g*Lambda(2,k));
        out( 1, k ) = (0.5*Lambda(1,k)*(2*Lambda(0,k)*Lambda(2,k) + pow(Lambda(1,k),2)* Lambda(2,k) + pow(Lambda(2,k),3)))/(_g*Lambda(2,k));
        out( 2, k ) = ((0.5*(2*Lambda(0,k)*Lambda(2,k) + pow(Lambda(1,k),2)* Lambda(2,k) + pow(Lambda(2,k),3)))/_g);
    }
}
...
Matrix ShallowWaterClosure2D::U( const Matrix& Lambda ) {
    ...
    for( unsigned k = 0; k < Lambda.columns(); ++k ) {
        y( 0, k ) = (0.5*(2*Lambda(0,k)*Lambda(2,k) + pow(Lambda(1,k),2)*Lambda(2,k) + pow(Lambda(2,k),3)))/(_g*Lambda(2,k));
        y( 1, k ) = (0.5*Lambda(1,k)*(2*Lambda(0,k)*Lambda(2,k) + pow(Lambda(1,k),2)* Lambda(2,k) + pow(Lambda(2,k),3)))/(_g*Lambda(2,k));
