...
Closure::Closure(Problem *problem): _problem(problem), _nMoments(_problem->GetNMoments()), _nQuadPoints(_problem->GetNQuadPoints()), _nStates(_problem->GetNStates())
{
    // initialize classes
...
blaze::DynamicMatrix<double> Closure::SolveClosure(const blaze::DynamicMatrix<double>& u, blaze::DynamicMatrix<double>& lambda){
    ...
    //std::cout<<"Entering Solve Closure with"<<std::endl;
    //std::cout<<u<<std::endl;
    ...
    if( CalcNorm(g) < _problem->GetEpsilon() ){
        //std::cout<<"TOLERANCE REACHED, exit"<<std::endl;
        return lambda;
    }
    ...
    for( int l=0; l<_problem->GetMaxIterations(); ++l ){
        ...
        if(l!=0){
            ...
            //std::cout<<"Norm is "<<CalcNorm(dlambda)<<std::endl;
        }
...
blaze::DynamicVector<double> Closure::EvaluateLambda(const blaze::DynamicMatrix<double>& lambda, int k){
    return lambda*_phiTilde[k];
}
