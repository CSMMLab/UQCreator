...
EulerClosure2D::EulerClosure2D( Problem* problem ) : Closure( problem ), _gamma( problem->GetGamma() ) { _alpha = 0.1; }
...
void EulerClosure2D::DU( Matrix& y, const Vector& Lambda ) {
    ...
    double v3      = Lambda[2];
    double v4      = Lambda[3];
    double expTerm = pow( -exp( ( ( pow( v2, 2 ) + pow( v3, 2 ) - 2.0 * v4 * ( v1 + _gamma ) ) / ( 2.0 * v4 ) ) ) * v4, ( 1.0 / ( 1.0 + _gamma ) ) ) *
                     1.0 / ( 1.0 + _gamma );
    ...
    y( 3, 3 )    = -( expTerm * ( pow( v2, 4 ) + pow( v3, 4 ) + 4.0 * pow( v3, 2 ) * v4 * _gamma - 4.0 * pow( v4, 2 ) * _gamma +
