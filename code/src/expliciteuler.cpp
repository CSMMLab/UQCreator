        ...
        for( unsigned j = 0; j < _mesh->GetNEdges(); ++j ) {
            ...
            if( _mesh->BoundaryAtEdge( j ) != NOSLIP && _mesh->BoundaryAtEdge( j ) != DIRICHLET ) {
                fluxFunc( _flux[j], _problem->G( uQ[I], uQ[J], _mesh->GetNormalsAtEdge( j ) / area, _mesh->GetNormalsAtEdge( j ), level ), level );
                            fluxFunc( tmp, _problem->G( uQ[J], uQ[I], -_mesh->GetNormalsAtEdge( j ) / area, -_mesh->GetNormalsAtEdge( j ), level ),
    level
                   ); std::cout << _flux[j] + tmp << std::endl; if( !std::isfinite( ( _flux[j] + tmp )( 0, 0 ) ) ) { std::cout << "flux = " <<
    _flux[j] << std::endl; std::cout << "uQI " << uQ[I] << std::endl; std::cout << "uQJ " << uQ[J] << std::endl; exit( EXIT_FAILURE );
                            }
