...
void RegularizedEuler::GradientNoRegularizaton( Vector& g, const Matrix& lambda, const Matrix& u, unsigned nTotal, unsigned nQTotal ) {
    Vector uKinetic( _nStates, 0.0 );
    g.reset();
...
void RegularizedEuler::SolveClosure( Matrix& lambda, const Matrix& u, unsigned nTotal, unsigned nQTotal ) {
    ...
    GradientNoRegularizaton( g, lambda, uF, nTotal, nQTotal );
