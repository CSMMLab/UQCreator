...
struct Closure
    ...
    function Closure(settings::Settings,basis::Basis,q::Quadrature)
        ...
        HQuad = zeros(q.Nq,settings.N,settings.N)
        for k = 1:q.Nq
            for i = 1:settings.N
                for j = 1:settings.N
        ...
        new(uPlus,uMinus,settings.N,q.Nq,settings.dx,settings.epsNewton,settings.NMaxNewton,basis,q,HQuad);
    end
end
...
function dL(obj::Closure,u::Array{Float64,1},v::Array{Float64,1})
    return IntegralMat(obj.q,0.5*(obj.basis.PhiQuad').*Ubb(obj,EvalAtQuad(obj.basis,v))') - u;
end

function ComputeMoments(obj::Closure,v::Array{Float64,1})
    return IntegralMat(obj.q,0.5*(obj.basis.PhiQuad').*Ubb(obj,EvalAtQuad(obj.basis,v))');
end

function ComputeMomentsWithQuad(obj::Closure,uQ::Array{Float64,1})
    return IntegralMat(obj.q,0.5*(obj.basis.PhiQuad').*uQ');
end
