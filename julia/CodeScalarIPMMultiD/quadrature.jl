...
struct Quadrature
   ...
   function Quadrature(Nq,quadType)
        if quadType == "Gauss"
            xi,w = QuadNodesWeights(Nq,-1.0,1.0)
        elseif quadType == "ClenshawCurtis"
            xi,w = QuadNodesWeightsClCu(Nq,-1.0,1.0)
        end
        xi = xi[end:-1:1];
        w = w[end:-1:1];
...
function Integral(q::Quadrature, f)
    fVals = zeros(q.Nq,q.Nq);
    w = zeros(q.Nq,q.Nq);
    for k = 1:q.Nq
        for l = 1:q.Nq
            fVals[(l-1)*q.Nq+k] = f(q.xi[k],q.xi[l])[1];
            w[(l-1)*q.Nq+k] = q.w[k]*q.w[l];
        end
    end
    return dot( w, fVals );
end
