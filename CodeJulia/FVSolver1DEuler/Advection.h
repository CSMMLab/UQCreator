/** Class defines physical problem as advection equation
 *
 *  @author Jonas Kusch
 ...
 *  CFL number added
 *  @version 1
 *  advection equation(first definition)
 */
...
class Advection : public PhysicalProblem
{
  private:
    const unsigned int _StateDim = 1;     ///< dimension of problem (number of conservation equations)
    const double _a              = 10;    ///< advection speed
  public:
    Advection( Settings* settings );    ///< constructor
    virtual ~Advection();               ///< destructor
